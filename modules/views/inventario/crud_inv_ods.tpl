            ...
            <table class="table table-bordered table-hover datatables">
              ...
              <tbody>
                ...
                <tr class="{estatus}">
                  ...
                  <td>{ods_pad} 
                    <span class="fas fa-info-circle _stats" data-title="{title}" data-body="{maquina} {marca} {modelo}<br><span style='font-size: 11px'>S/N: {serial}</span>"></span>
                  </td>
                  <td>{almacen}</td>
                  <td>{fecha_mov}</td>
                  <td>{articulos}</td>
...
<script>
  jQuery("._stats").each(function(){
    jQuery(this).popover({
      title: '<div style="font-size: 12px;">CLIENTE: <strong>'+jQuery(this).attr("data-title")+'</strong></div>',
