          ...
          <div class="table-responsive">
            <table class="table table-bordered table-hover datatables" data-dt_order='[[0,"desc"]]'>
              ...
              <tbody>
                ...
                <tr class="{estatus}">
                  <td>{codigo}</td>
                  <td><span class="_stats" data-ods="{ods_pad}" data-cuerpo="{cuerpo}">{ods_pad}</span></td>
                  <td>{code}</td>
                  <td>{data}</td>
                  <td>{fecha_orden}</td>
                  <td>{articulos}</td>
...
<script>
  jQuery("._stats").each(function(){
    jQuery(this).popover({
      title: '<div style="font-size: 12px;">ODS: <strong>'+jQuery(this).attr("data-ods")+'</strong></div>',
