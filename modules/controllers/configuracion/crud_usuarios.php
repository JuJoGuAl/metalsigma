	...
	if(Evaluate_Mod($modulo)){
		...
		if($data["title"]=="SUCCESS"){
			foreach ($data["content"] as $llave => $datos) {
				...
				<button type="button" class="btn btn-outline-secondary btn-circle btn-sm waves-effect waves-light" data-toggle="tooltip" data-placement="top" title="CAMBIAR ESTATUS" data-menu="'.$var_array_nav["mod"].'" data-mod="'.$var_array_nav["submod"].'" data-ref="'.$var_array_nav["ref"].'" data-subref="'.$var_array_nav["subref"].'" data-acc="STATUS" data-id="'.$id.'"><i class="fas fa-sync"></i></button>
				';
				...
				$stats_color = ($datos['status']==1) ? "table-success" : "table-danger" ;
