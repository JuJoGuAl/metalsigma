...
if($perm_val["title"]<>"SUCCESS"){
	...
	if(Evaluate_Mod($modulo)){
		...
		foreach ($modulo["content"] as $key => $value){
			...
			$tpl->assign("menu_name","ORDENES DE COMPRAS POR ODS");
		}
		$data=$data_class->list_odc(false,false,false,false,true);
			...
			foreach ($data["content"] as $llave => $datos) {
				...
				if($datos['ods_pad']!="N/A"){
					...
					if($detalles["title"]=="SUCCESS"){
						...
						$cuerpo .= "EQUIPO: <strong>".$cab["maquina"]." ".$cab["marca"]." ".$cab["modelo"]."</strong><br>";
						$cuerpo .= "S/N: <strong>".$cab["serial"]."</strong><br>";
						$cuerpo .= "SEGMENTO: <strong>".$cab["segmento"]."</strong><br>";
					}
				}
