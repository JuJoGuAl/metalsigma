...
if($action=="save_new" || $action=="save_edit" || $action=="proc"){
	...
	if(isset($_SESSION["metalsigma_log"])){
		...
		if($perm_val["title"]<>"SUCCESS"){
			...
			$datos = $detalles = $det_det = $det_art = $det_cant = $det_costou = $det_impp = $det_impm = $det_total = $det_cot_det = $det_rest = array();			
			$status_ = ($action=="proc") ? "PRO" : "PEN";
			array_push($datos, $_cproveedor);
			array_push($datos, setDate($_fecha));
			array_push($datos, $_cods);//ODS
			...
			if($action=="save_edit" || $action=="proc"){
				if($upt!=1){
					...
					if($_id==0){
						$resultado=$data_class->new_odc($datos,$detalles);
					}else{
						$resultado=$data_class->edit_odc($_id,$datos,$detalles);
					}
				}
			}else{
				if($ins!=1){
			...
			switch ($action) {
				case "save_new":
					$mensaje="ODC CREADA CON EXITO";
				break;
				case "save_edit":
					$mensaje="DATOS DE LA ODC ACTUALIZADOS";
				break;
				case "proc":
					$mensaje="ODC PROCESADA!";
				break;
			}
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["ref"]="FORM_ODC_ODS";
				...
				$tpl->assign("menu_name","ORDENES DE COMPRAS CON ODS");
			...
			if($action=="new"){
				...
				$tpl->assign("codigo",0);
				$tpl->assign("fecha_orden",date("d-m-Y"));
			}elseif($action=="edit"){
				$tpl->assign("accion",'save_edit');
				$data=$data_class->get_odc($_GET['id']);
