...
if($perm_val["title"]<>"SUCCESS"){
	...
	if(Evaluate_Mod($modulo)){
		...
		foreach ($modulo["content"] as $key => $value){
			...
			$tpl->assign("menu_name","CONSUMOS PARA ODS");
		}
		$data=$data_class->list_mov("CSM",false,false,false,false,false,$almacenes);
		if($data["title"]=="SUCCESS"){
			foreach ($data["content"] as $llave => $datos) {
				...
				foreach ($data["content"][$llave] as $key => $value){
					...
					$tpl->assign("title",$data["content"][$llave]["cot_cliente"]." (".formatRut($data["content"][$llave]["cot_code"]).")");
					$tpl->assign("cuerpo",$stats_color);
				}
			}			
		}
