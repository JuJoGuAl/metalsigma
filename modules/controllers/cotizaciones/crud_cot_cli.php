...
if($perm_val["title"]<>"SUCCESS"){
	...
	if(Evaluate_Mod($modulo)){
		...
		$data=$data_class->list_all($array_cot_cli);
		if($data["title"]=="SUCCESS"){
			foreach ($data["content"] as $llave => $datos) {
				...
				$value = ($key=="code") ? formatRut($value) : $value ;
				$detalles=$data_class->list_sub($datos['codigo'],$array_cot_cli);
				$sub_status = "";
				$contador = 0;
				$class = "";
				if($detalles["title"]=="SUCCESS"){
					foreach ($detalles["content"] as $llave1 => $datos1) {
						...
						$class = ($datos1["ctipo"]==5 && ($datos1["status"]=="PCL")) ? "table-warning" : $class ;
