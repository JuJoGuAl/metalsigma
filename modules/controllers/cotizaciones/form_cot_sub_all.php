			...
			if($action=="save_new" || $action=="save_edit" || $action=="proc"){
				...
				if($action=="save_edit" || $action=="proc"){
					if($upt!=1){
						...
						if($_id==0){
							$resultado=$data_class->new_co_sub($datos,$detalles,$articulos,$cotizaciones);
						}else{
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["ref"]="FORM_COT_SUB_ALL";
				$var_array_nav["subref"]="CRUD_COT_SUB_ALL";
			...
			if($action=="new"){
				...
				$codigo_origen=$datos["content"]["codigo"];
				...
				foreach ($lugares["content"] as $key => $value) {
					...
					if($value["codigo"]==1){// default Taller
				...
				if(!empty($equipost)){
					foreach ($equipost["content"] as $llave => $datos){
						...
						if($datos["codigo"]==1){// default Tec + Ayudante
				...
				$data_ar=$param->list_a($csegmento);
				$data_VE=$param->get_v(1);
