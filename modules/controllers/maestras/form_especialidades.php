		...
		if($perm_val["title"]<>"SUCCESS"){
			...
			if($action=="save_edit"){
				if($upt!=1){
					...
					$resultado=$data_class->edit_esp($_id,$datos);
				}
			}else{
				if($ins!=1){
					$resultado['title']="ERROR";
					...
					$resultado=$data_class->new_esp($datos);
				}
			}
			...
			switch ($action) {
				...
				break;
				case "save_edit":
					$mensaje="DATOS DE lLA ESPECIALIDAD ACTUALIZADOS";
				break;
			}
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["ref"]="FORM_ESPECIALIDADES";
				$var_array_nav["subref"]="CRUD_ESPECIALIDADES";
				...
				$tpl->assign("menu_name","ESPECIALIDADES DE TRABAJADORES");
			}
			if($action=="new"){
				$tpl->assign("accion",'save_new');
			}elseif($action=="edit"){
				$tpl->assign("accion",'save_edit');
				$data=$data_class->get_esp($_GET['id']);
