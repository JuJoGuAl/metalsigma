		...
		if($perm_val["title"]<>"SUCCESS"){
			...
			if($action=="save_edit"){
				...
				if($ins!=1){
					...
					$resultado=$data_class->new_e($datos);
				}
			}
			...
			switch ($action) {
				case "save_new":
					$mensaje="EQUIPO DE TRABAJO CREADO CON EXITO";
				break;
				case "save_edit":
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["ref"]="FORM_PAR_EQUI";
				$var_array_nav["subref"]="CRUD_PAR_EQUI";
				...
				$tpl->assign("mod_name","EQUIPOS DE TRABAJO");
			}
			if($action=="new"){
				$tpl->assign("accion",'save_new');
			}elseif($action=="edit"){
				$tpl->assign("accion",'save_edit');
				$data=$data_class->get_e($_GET['id']);
