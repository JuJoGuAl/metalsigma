		...
		if($perm_val["title"]<>"SUCCESS"){
			...
			if($action=="save_edit"){
				if($upt!=1){
					...
					$resultado=$data_class->edit_car($_id,$datos);
				}
			}else{
				if($ins!=1){
					$resultado['title']="ERROR";
					...
					$resultado=$data_class->new_car($datos);
				}
			}
			...
			switch ($action) {
				...
				break;
				case "save_edit":
					$mensaje="DATOS DEL CARGO ACTUALIZADOS";
				break;
			}
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["ref"]="FORM_CARGOS";
				$var_array_nav["subref"]="CRUD_CARGOS";
				...
				$tpl->assign("menu_name","CARGOS DE TRABAJADORES");
			}
			if($action=="new"){
				$tpl->assign("accion",'save_new');
			}elseif($action=="edit"){
				$tpl->assign("accion",'save_edit');
				$data=$data_class->get_car($_GET['id']);
