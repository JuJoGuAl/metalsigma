		...
		if($perm_val["title"]<>"SUCCESS"){
			...
			if($action=="save_edit"){
				if($upt!=1){
					...
					$resultado=$data_class->edit_g($_id,$datos);
				}
			}else{
				if($ins!=1){
					$resultado['title']="ERROR";
					...
					$resultado=$data_class->new_g($datos);
				}
			}
			...
			switch ($action) {
				case "save_new":
					$mensaje="GIRO CREADO CON EXITO";
				break;
				case "save_edit":
					$mensaje="DATOS DEL GIRO ACTUALIZADOS";
				break;
			}
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["ref"]="FORM_GIROS";
				$var_array_nav["subref"]="CRUD_GIROS";
				...
				$tpl->assign("menu_name","GIROS");
				$tpl->assign("mod_name","FORMULARIO DE GIROS");
			}
			if($action=="new"){
				$tpl->assign("accion",'save_new');
			}elseif($action=="edit"){
				$tpl->assign("accion",'save_edit');
				$data=$data_class->get_g($_GET['id']);
