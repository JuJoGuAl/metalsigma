...
if($action=="save_new" || $action=="save_edit"){
	...
	if(isset($_SESSION["metalsigma_log"])){
		...
		if($perm_val["title"]<>"SUCCESS"){
			...
			$datos=array();
			array_push($datos, $_code);
			array_push($datos, $_dato);
			array_push($datos, $_cparte);
			...
			if($action=="save_edit"){
				if($upt!=1){
					...
					$resultado=$data_class->edit_pi($_id,$datos);
				}
			}else{
				if($ins!=1){
					$resultado['title']="ERROR";
					...
					$resultado=$data_class->new_pi($datos);
				}
			}
			...
			switch ($action) {
				case "save_new":
					$mensaje="COMPONENTE CREADO CON EXITO";
				break;
				case "save_edit":
					$mensaje="DATOS DEL COMPONENTE ACTUALIZADOS";
				break;
			}
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["ref"]="FORM_COMPONENTES";
				$var_array_nav["subref"]="CRUD_COMPONENTES";
				...
				$tpl->assign("menu_name","COMPONENTES");
				$tpl->assign("mod_name","FORMULARIO DE COMPONENTES");
			}
			if($action=="new"){
				$tpl->assign("accion",'save_new');
			}elseif($action=="edit"){
				$tpl->assign("accion",'save_edit');
				$data=$data_class->get_pi($_GET['id']);
