		...
		if($perm_val["title"]<>"SUCCESS"){
			...
			if($action=="save_edit"){
				if($upt!=1){
					...
					$resultado=$data_class->edit_pa($_id,$datos);
				}
			}else{
				if($ins!=1){
					$resultado['title']="ERROR";
					...
					$resultado=$data_class->new_pa($datos);
				}
			}
			...
			switch ($action) {
				case "save_new":
					$mensaje="SISTEMA CREADO CON EXITO";
				break;
				case "save_edit":
					$mensaje="DATOS DEL SISTEMA ACTUALIZADOS";
				break;
			}
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["ref"]="FORM_SISTEMAS";
				$var_array_nav["subref"]="CRUD_SISTEMAS";
				...
				$tpl->assign("menu_name","SISTEMAS");
				$tpl->assign("mod_name","FORMULARIO DE SISTEMAS");
			}
			if($action=="new"){
				$tpl->assign("accion",'save_new');
			}elseif($action=="edit"){
				$tpl->assign("accion",'save_edit');
				$data=$data_class->get_pa($_GET['id']);
