		...
		if($perm_val["title"]<>"SUCCESS"){
			...
			if($action=="save_edit"){
				if($upt!=1){
					...
					$resultado=$data_class->edit_pag($_id,$datos);
				}
			}else{
				if($ins!=1){
					$resultado['title']="ERROR";
					...
					$resultado=$data_class->new_pag($datos);
				}
			}
			...
			switch ($action) {
				case "save_new":
					$mensaje="FORMA DE PAGO CREADA CON EXITO";
				break;
				case "save_edit":
					$mensaje="DATOS DEL PAGO ACTUALIZADOS";
				break;
			}
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["ref"]="FORM_PAGOS";
				$var_array_nav["subref"]="CRUD_PAGOS";
				...
				$tpl->assign("menu_name","PAGOS");
				$tpl->assign("mod_name","FORMULARIO DE PAGOS");
			}
			if($action=="new"){
				$tpl->assign("accion",'save_new');
			}elseif($action=="edit"){
				$tpl->assign("accion",'save_edit');
				$data=$data_class->get_pag($_GET['id']);
