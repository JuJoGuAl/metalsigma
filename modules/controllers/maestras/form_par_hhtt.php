...
if($action=="save_new" || $action=="save_edit"){
	...
	if(isset($_SESSION["metalsigma_log"])){
		...
		if($perm_val["title"]<>"SUCCESS"){
			...
			array_push($datos, $_sitio);
			array_push($datos, $_equipo);
			array_push($datos, $_valor);
			array_push($datos, $_normal);
			array_push($datos, $_extra);
			...
			switch ($action) {
				case "save_new":
					$mensaje="VALOR DE HORA/HOMBRE CREADA CON EXITO";
				break;
				case "save_edit":
					$mensaje="DATOS DE LA HORA/HOMBRE ACTUALIZADOS";
				break;
			}
	...
	if($perm_val["title"]<>"SUCCESS"){
		...
		if(Evaluate_Mod($modulo)){
			...
			foreach ($modulo["content"] as $key => $value){
				...
				$var_array_nav["subref"]="CRUD_PAR_HHTT";
			...
			if($action=="new"){
				...
				$lugar = $data_class->list_l();
				if(!empty($lugar)){
					foreach ($lugar["content"] as $key => $value) {
						$tpl->newBlock("lug_det");
						foreach ($lugar["content"][$key] as $key1 => $value1){
							$tpl->assign($key1,$value1);
						}
					}
				}
				$equipo = $data_class->list_e();
				if(!empty($equipo)){
					foreach ($equipo["content"] as $key => $value) {
						$tpl->newBlock("equi_det");
						foreach ($equipo["content"][$key] as $key1 => $value1){
							$tpl->assign($key1,$value1);
						}
					}
				}
				...
				if(Evaluate_Mod($data)){
					...
					if(!empty($equipo)){
						foreach ($equipo["content"] as $key => $value) {
							$tpl->newBlock("equi_det");
							if($value["codigo"]==$cab["cequipotrab"]){
								$tpl->assign("selected",$selected);
							}
							foreach ($equipo["content"][$key] as $key1 => $value1){
								$tpl->assign($key1,$value1);
